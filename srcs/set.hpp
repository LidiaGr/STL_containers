...
namespace ft {
	...
	protected:
		...
		iterator _checkBounds(_tNode *node, const key_type& k, bool isLower) const {
			if (!node || node == _preFirstNode || node == _postLastNode)
				return iterator(_postLastNode);

			bool less = _comp(k, *node->_data);
			bool greater = _comp(*node->_data, k);
			if (!less && !greater) {
				if (!isLower)
	...
	public:
		...
		set (InputIterator first, InputIterator last,
			 ...
			 typename enable_if<std::__is_input_iterator<InputIterator>::value>::type* = 0) : _root(nullptr), _size(0), _allocData(alloc), _comp(comp) {
			_createTheoreticalNodes();
		...
		set (const set& node) : _root(nullptr), _size(0) {
			_createTheoreticalNodes();
			*this = node;
		};
		...
		std::pair<iterator, bool> insert (const value_type& val) {
			...
			while (current != nullptr && current != _postLastNode && current != _preFirstNode) {
				if (val == *current->_data) return std::make_pair(iterator(current), false);
				parent = current;
				current = (val < *current->_data) ? current->_left : current->_right;
			}
		...
		iterator lower_bound (const key_type& k) const { return _checkBounds(_root, k, false); };
		iterator upper_bound (const key_type& k) const { return _checkBounds(_root, k, true); };
		std::pair<iterator,iterator> equal_range (const key_type& k) const { return std::make_pair(lower_bound(k), upper_bound(k)); };
